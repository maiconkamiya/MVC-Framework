    ...
    switch (element.tipo){
        ...
        default:
            if (element.flagBotaoForm == 'S'){
                ...
                field = `<div class="input-group"><input type="text" class="form-control ${element.flagEditavel == 'S' ? 'f-input' : ''} ${element.classIndex}" name="${element.coluna}" value="${value}" style="max-width: 120px" autocomplete="off" ${element.flagObrigatorio == 'S' ? 'required' : ''}><button class="btn btn-outline-secondary" type="button" onclick="${element.btnFuncao}"><i class="fas fa-ellipsis-h"></i></button><input type="text" class="form-control ${element.classIndex}" name="label" disabled></div>`;
            } else {
